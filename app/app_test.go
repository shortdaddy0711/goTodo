...
func TestTodos(t *testing.T) {
	...
	// resp, err = http.PostForm(ts.URL+"/api/todos", v2)
	// assert.NoError(err)
	// defer resp.Body.Close()
	// assert.Equal(http.StatusCreated, resp.StatusCode)
	// err = json.NewDecoder(resp.Body).Decode(&todo)
	...
	// resp, err = http.Get(ts.URL + "/api/complete/" + id1.String() + "?complete=true")
	// assert.NoError(err)
	// assert.Equal(http.StatusOK, resp.StatusCode)
	// resp, err = http.Get(ts.URL + "/api/todos")
	// assert.NoError(err)
	...
	// req, _ := http.NewRequest("DELETE", ts.URL+"/api/todos/"+id1.String(), nil)
